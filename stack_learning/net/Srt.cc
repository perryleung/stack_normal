...
public:
	...
	void Init()
	{
		...
		//YAML::Node entry5 = Config::Instance()["Srt"]["entry5"];//一般要用几个节点就读几个节点
	...
	/**
  	* @brief: 在等待状态接受到上层发来的数据时能第一时间记录数据到达的时间
  	*/
	bool WriteTrace(const Ptr<MsgSendDataReq> &m);

	void NothingToDo(const Ptr<MsgSendDataReq> &m);	
private:
...
struct WaitRsp : hsm::State<WaitRsp, Top>
{
	...
	HSM_TRANSIT_DEFER(MsgSendDataReq,Idle,&Srt::NothingToDo,&Srt::WriteTrace);
...
void Srt::NothingToDo(const Ptr<MsgSendDataReq> &m)//什么都不做
{

};
bool Srt::WriteTrace(const Ptr<MsgSendDataReq> &m)
{
	LOG(INFO)<<"在等待状态接收到上层发来到数据"<<endl;
	if(m->Write==true)//已经写入过trace了，不需要再写
	{
		return false;
	}
	...
	{
		m->Write=true;
		SrtHeader *header = m->packet.Header<SrtHeader>();
		LOG(INFO)<<"sendDown()...m->address"<<m->address;
		uint8_t dest = (uint8_t)m->address;
		if(handleHead(header, dest)){
		string logStr = Trace::Instance().Log(NET_LAYER, NET_PID, m->packet, (int)header->srcID, -1,  (int)header->nextID,  (int)header->destID, -1, -1, -1, -1, "data", "send");
		if (TraceClient != NULL)
		{
			LOG(INFO)<<"在等待状态写数据"<<endl;
	...
	if(header->nextID==full.getNodeID()&&header->destID==full.getNodeID())
		{
			...
			string logStr = Trace::Instance().Log(NET_LAYER, NET_PID, msg->packet, (int)header->srcID, -1, (int)header->nextID , (int)header->destID, -1, -1, -1, -1, "data", "sendUp");
			if (TraceClient != NULL)
			{
				IODataPtr pkt(new IOData);
...
void Srt::SendDown1(const Ptr<MsgSendDataReq> &msg)
{
	...
	if(handleHead(header, dest)){
		...
		{
			msg->Write=true;
		string logStr = Trace::Instance().Log(NET_LAYER, NET_PID, msg->packet, (int)header->srcID, -1,  (int)header->nextID,  (int)header->destID, -1, -1, -1, -1, "data", "send");
		if ((TraceClient != NULL))
		{
			IODataPtr pkt(new IOData);
...
void Srt::SendDown2(const Ptr<MsgRecvDataNtf> &msg)
{
	...
	if(handleHead(header)){
		string logStr = Trace::Instance().Log(NET_LAYER, NET_PID, msg->packet, (int)header->srcID, -1,  (int)header->nextID,  (int)header->destID, -1, -1, -1, -1, "data", "forward");
		if (TraceClient != NULL)
		{
			IODataPtr pkt(new IOData);
