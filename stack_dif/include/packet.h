...
/*?Packet???ref_???unsigned???????start_???char*?????????????????????????????????????ref_?start_??????????????????????????????????????????????????????*/
...
/*??????????????????????????????????????????ref_?start_?delete????????????????????*/
    ...
    {
        // if packet â†“, we don't know where is head
        if (DOWN == dir_) {
...
//Packet???????????????????????????????????????????????
    ...
    {
        // if packet â†‘, we don't know where is data
        if (UP == dir_) {
    ...
    {
        // if packet â†‘, we don't know where is tail
        if (UP == dir_) {
